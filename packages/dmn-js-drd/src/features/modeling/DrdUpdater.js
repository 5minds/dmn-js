...
export default function DrdUpdater(
  ...
  eventBus.on('resize.start', function(event) {
    ...
    if (is(businessObject, 'dmn:DecisionService')) {
      ...
      const decisions = businessObject.$parent.get('drgElement')
        .filter(d => is(d, 'dmn:Decision'));
      const encapsulated = decisions.filter(
        d => businessObject.get('encapsulatedDecision').some(e => e.href
      === '#' + d.id)
      );
      const encapsulatedWithoutOutput = encapsulated.filter(
        e => !businessObject.get('outputDecision').some(o => o.href === '#' + e.id)
      );
      const output = decisions.filter(
        d => businessObject.get('outputDecision').some(o => o.href === '#' + d.id)
      );
      ...
      if (isSouth) {
        const highestOutputDecisionY = Math.max(...output
          .map(d => d.di.bounds.y + d.di.bounds.height
          ));

        minBounds.height = Math.max(
          minBounds.height,
          (highestOutputDecisionY - shape.y) * 2 + DEFAULT_CHILD_BOX_PADDING
        );
      }

      if (isNorth) {
        const lowestEncapsulatedDecisionY = Math.min(...encapsulatedWithoutOutput
          .map(d => d.di.bounds.y));

        minBounds.height = Math.max(
          minBounds.height,
          (shape.y + shape.height - lowestEncapsulatedDecisionY)
          * 2
          + DEFAULT_CHILD_BOX_PADDING
        );

        minBounds.y = Math.min(
          minBounds.y,
          lowestEncapsulatedDecisionY - DEFAULT_CHILD_BOX_PADDING - minBounds.height / 2
        );
      }
